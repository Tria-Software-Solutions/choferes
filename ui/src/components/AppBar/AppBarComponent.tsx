        ...
        {!isMobile && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            >
              ...
              >
                {links.length === 0 ? (
                  <Box sx={{ color: 'white', p: 1 }}>No hay links disponibles</Box>
                ) : (
        ...
        >
          {userLinks.map((link) => (
            <MenuItem 
              ...
              sx={link.label === APPBAR_MENU.LOGOUT ? { color: theme.palette.error.main } : {}}
            >
              <ListItemIcon>
                {React.cloneElement(link.icon as React.ReactElement, {
                  fontSize: "small",
                  color: link.label === APPBAR_MENU.LOGOUT ? "error" : "inherit"
                })}
              </ListItemIcon>

...
export function validateName(value: string): string {
  ...
  if (!nameRegex.test(value)) return "Solo se permiten letras, espacios y guiones";
...
export function validateUsername(value: string): string {
  if (!value.trim()) return "Este campo es requerido";
  if (!usernameRegex.test(value)) return "Debe empezar con letra y contener solo letras, números, puntos y guiones bajos";
  return "";
}

export function validatePassword(value: string): string {
  if (!value.trim()) return "Este campo es requerido";
  if (!passwordRegex.test(value)) return "Mínimo 8 caracteres, una mayúscula, una minúscula, un número y un carácter especial";
  return "";
}
